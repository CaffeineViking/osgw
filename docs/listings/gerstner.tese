...
// are interpolated bilineably (for a quad) to find the new vertex properties. And, we
...
void main() {
    ...
    vec2 x_up_texture_mix = mix(te_in[0].texture_coordinate,
                                te_in[3].texture_coordinate,
                                gl_TessCoord.x);
    vec2 x_down_texture_mix = mix(te_in[1].texture_coordinate,
                                  te_in[2].texture_coordinate,
                                  gl_TessCoord.x);
    te_out.texture_coordinate = mix(x_down_texture_mix,
                                    x_up_texture_mix,
                                    gl_TessCoord.y);
    ...
    // Below is where all the magic happens, we displace the vertex positions:
